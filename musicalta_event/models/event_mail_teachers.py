...
class EventMailTeacherScheduler(models.Model):
    """ Event automated mailing. This model replaces all existing fields and
    configuration allowing to send emails on events since Odoo 9. A cron exists
    that periodically checks for mailing to run. """
    _name = 'event.mail.teacher'
    _rec_name = 'event_id'
    _description = 'Event Automated Mailing for teachers'
    ...
    def set_template_ref_model(self):
        ...
        if self.notification_type == 'mail':
            record = mail_model.search(
                [('model', '=', 'event.mail.teacher.registration')], limit=1)
            self.template_ref = "{},{}".format(
                'mail.template', record.id) if record else False
    ...
    scheduled_date = fields.Datetime(
        'Schedule Date', store=True)
    # contact and status
    mail_registration_ids = fields.One2many(
        'event.mail.teacher.registration', 'scheduler_id',
    ...
    template_ref = fields.Reference(
        ...
        domain="[('model', '=', 'event.mail.teacher.registration')]")
    ...
    @api.depends('scheduled_date', 'mail_done')
    ...
    def execute(self):
        for scheduler in self:
            ...
            if scheduler.template_ref:
                ...
                interval = _INTERVALS.get(scheduler.interval_unit)
                next_scheduled_date = now + interval(scheduler.interval_nbr)
                scheduler.update({
    ...
    def _prepare_event_mail_values(self):
        self.ensure_one()
        return namedtuple("MailValues", ['notification_type', 'interval_nbr', 'interval_unit', 'template_ref'])(
    ...
    def _warn_template_error(self, scheduler, exception):
        ...
        if random.random() < 0.1666 or scheduler.interval_unit in ('now', 'hours'):
            ...
            try:
                ...
                subject = _(
                    "WARNING: Event Scheduler Error for event: %s", event.name)
...
class EventMailTeacherRegistration(models.Model):
    ...
    scheduler_id = fields.Many2one(
        'event.mail.teacher', 'Mail Scheduler', required=True, ondelete='cascade')
    ...
    def execute(self):
        ...
        for reg_mail in todo:
            if reg_mail.teacher_id not in done.mapped('teacher_id'):
                ...
                if not template:
                    _logger.warning("Cannot process ticket %s, because Mail Scheduler %s has reference to non-existent template",
    ...
    @api.depends('scheduler_id.scheduled_date')
    def _compute_scheduled_date(self):
        for mail in self:
            if mail.scheduler_id.scheduled_date:
                mail.scheduled_date = mail.scheduler_id.scheduled_date
            else:
